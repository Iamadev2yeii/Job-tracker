...
3. Open the unzipped `job-scraper` folder on your computer, select
   **everything inside it** (not the `job-scraper` folder itself —
   ...
   > `config/companies.yaml` has 161 Munich-area companies, deliberately
   > — the more companies covered, the more likely a solid chunk work
   > cleanly on the first try even before you fix anything. See Part 5
   > for what "some fail" looks like and why that's expected, not a
...
## Part 11 — Why this is Munich-only

This tracker only ever checks one area: Munich plus its immediate
commuter towns (Ottobrunn, Freising, Garching, Dachau, and similar —
see `src/matcher.py`, `MUNICH_KEYWORDS`), not the whole of Bavaria and
not any other city. There's just one sheet ("Jobs") in the Excel file
and one section on the webpage — no separate "Dream Cities" tier.

If you ever want to widen this to more cities later, the code that
does that (multi-city matching, multiple tracker sheets, a country
column) already exists in the original version of this tracker — it
was deliberately stripped out here to keep things simple for a
Munich-only search, not because it's hard to add back.

If you want to turn a score floor on, adjust `main_min_score` in
`config/cv_profile.yaml` (0 = off, the default).
...
all 161 working to get value from this.
